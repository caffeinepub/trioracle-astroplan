...
interface UPIPaymentSectionProps {
  amount: number;
  /** Label shown above the amount, e.g. "One Question Service" */
  serviceLabel?: string;
}
...
export default function UPIPaymentSection({
  ...
  return (
    <div className="rounded-xl border border-gold/25 bg-gradient-to-b from-amber-50/70 to-white/80 p-5 space-y-4">
      ...
      {/* QR code + details side-by-side on sm+, stacked on xs */}
      <div className="flex flex-col sm:flex-row items-center gap-5">
        ...
        <div className="flex-1 space-y-3 w-full">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-white/70 border border-gold/20 rounded-lg px-3 py-2">
              ...
              <button
                ...
                onClick={handleCopy}
                title="Copy UPI ID"
                className={`flex items-center gap-1 text-xs px-2 py-1 rounded-md transition-all duration-200 border ${
                  copied
                    ? "bg-sage/10 border-sage/30 text-sage"
                    : "bg-gold/8 border-gold/20 text-gold-dark hover:bg-gold/15"
                }`}
              >
                {copied ? (
                  <>
          ...
          <ul className="text-xs text-charcoal/55 space-y-1 leading-relaxed">
            ...
            <li className="flex items-start gap-1.5">
              <span className="text-gold-dark mt-0.5">✦</span>
              Complete payment, then submit this inquiry form
            </li>
          </ul>
        </div>
      </div>
