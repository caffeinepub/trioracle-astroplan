...
import type { Post, Service, Inquiry, UserProfile } from '../backend';
...
export function useSubmitInquiry() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Handle single hand picture blob (legacy)
      ...
      // Handle multiple palm photos (new special service)
        ...
        for (let i = 0; i < totalFiles; i++) {
          ...
          if (onProgress) {
            ...
            blob = blob.withUploadProgress((pct) => {
              ...
              const overall = Math.round(((fileIndex + pct / 100) / totalFiles) * 100);
              onProgress(overall);
            });
          }
