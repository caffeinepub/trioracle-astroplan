...
function PlaceOfBirthSection({
  ...
  onChange: (updated: PlaceOfBirthFields) => void;
}) {
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => onChange({ ...fields, country: e.target.value })}
            placeholder="Country"
            className="text-sm h-9 border-gold/20 focus:border-gold/50"
          />
        </div>
...
function PersonSection({
  ...
  prefix: string;
  fields: PersonFields;
  onChange: (updated: PersonFields) => void;
  required?: boolean;
}) {
  return (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitMutation.mutateAsync({
        ...
        birthCity: placeOfBirth.city.trim() || null,
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Share any significant past life events or experiences relevant to your query..."
              rows={2}
              className="text-sm border-gold/20 focus:border-gold/50 resize-none"
            />
          </div>
