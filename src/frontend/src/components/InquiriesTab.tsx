...
function ZoomableImageModal({
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      link.download = `${alt.replace(/\s+/g, "_")}.${ext}`;
    ...
    >
      ...
      >
        <button
          ...
          className="p-1.5 rounded-full bg-white/20 hover:bg-white/30 text-white disabled:opacity-30 transition-colors"
          title="Zoom out"
        >
          <ZoomOut size={16} />
        </button>
      ...
      {/* Extra children (e.g. navigation buttons, caption) */}
      {children}
...
function HandPicturePreview({ inquiry }: { inquiry: Inquiry }) {
  ...
  const name = inquiry.visitorName || "hand_picture";
    ...
    <>
      ...
      {fullViewIndex !== null && (
        <ZoomableImageModal
          ...
          alt={`${name}_palm_photo_${fullViewIndex + 1}`}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      ...
                      >
                        {serviceNames[Number(inquiry.serviceType)] ||
        ...
        {inquiries.map((inquiry) => {
          ...
          return (
            ...
            >
              ...
              {/* Email — shown prominently below name */}
              {parsedEmail && (
